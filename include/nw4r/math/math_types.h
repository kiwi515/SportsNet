...
inline void VEC3Add(register VEC3* destVec, register const VEC3* srcVec1,
                    register const VEC3* srcVec2) {
    asm
    {
...
inline void VEC3Sub(register VEC3* destVec, register const VEC3* srcVec1,
                    register const VEC3* srcVec2) {
    asm
    {
				psq_l f2, 0x0(srcVec1), 0, 0
...
inline void VEC3Scale(register VEC3* outVec, register const VEC3* inVec,
...
struct VEC3 {
    ...
    inline bool operator!=(const VEC3& other) const {
        return mCoords.x != other.mCoords.x || mCoords.y != other.mCoords.y ||
               mCoords.z != other.mCoords.z;
    }
...
struct MTX33 {
    ...
    inline operator MtxRef() { return mEntries.tbl; }

    inline operator MtxRefConst() const { return mEntries.tbl; }
};
...
inline void MTX34Inv(MTX34* out, const MTX34* in) { PSMTXInverse(*in, *out); }
...
inline void MTX34Identity(MTX34* mtx) { PSMTXIdentity(*mtx); }

inline void MTX34Copy(MTX34* out, const MTX34* in) { PSMTXCopy(*in, *out); }
...
inline void MTX34Scale(MTX34* outMtx, const VEC3* vec, const MTX34* inMtx) {
    PSMTXScaleApply(*inMtx, *outMtx, vec->mCoords.x, vec->mCoords.y,
                    vec->mCoords.z);
}
...
inline float FSqrt(float f) { return f * FrSqrt(f); }

inline float VEC3Len(const VEC3* in) { return PSVECMag(*in); }
