...
inline u32 GetResColorAnmResult(const ResColorAnmData* pData, float time,
                                bool b) {
    if (b)
        return pData->mColor;

    return GetResColorAnmResult(
        (const ResColorAnmFramesData*)((u8*)pData + pData->mOffset), time);
}
...
template <typename T> inline float ClipFrame(T& info, float time) {
    if (time <= 0.0f)
        return 0.0f;

    if (info.MAX_0x8 <= time)
        return info.MAX_0x8;
...
inline float GetResAnmResult(const ResAnmData* pData, float time, bool b) {
    ...
    return GetResKeyFrameAnmResult(
        (const ResKeyFrameAnmData*)((u8*)pData + pData->mOffset), time);
}
} // namespace detail
} // namespace g3d
} // namespace nw4r
