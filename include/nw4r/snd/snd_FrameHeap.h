...
struct FrameHeap {
    ...
    struct Block {
        ...
        inline Block(u32 size, AllocCallback callback, void* ptr)
            : mSize(size), mCallback(callback), PTR_0x10(ptr) {}

        inline void* GetBufferAddr() {
            return reinterpret_cast<u8*>(this) +
                   ut::RoundUp<u32>(sizeof(Block), 0x20);
        }
    ...
    struct Section {
        ...
        inline void AppendBlock(Block* pBlock) { mList.PushBack(pBlock); }
    ...
    inline bool IsValid() const { return mList.GetSize(); }
};
} // namespace detail
} // namespace snd
} // namespace nw4r
