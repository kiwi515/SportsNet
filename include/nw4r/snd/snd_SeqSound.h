...
struct SeqSound : BasicSound {
    ...
    struct SeqLoadTask : Task {
        ...
        inline SeqLoadTask()
            : mFileStream(NULL), mData(NULL), mCallback(NULL), mSound(NULL) {}
    ...
    inline const BasicPlayer* GetBasicPlayer() const { return &mPlayer; }

    inline BasicPlayer* GetBasicPlayer() { return &mPlayer; }

    inline bool IsPrepared() const { return mPreparedFlag; }
