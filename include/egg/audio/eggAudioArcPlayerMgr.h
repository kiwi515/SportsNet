...
public:
    ...
    virtual UNKTYPE* openArchive(const char*, nw4r::snd::SoundHeap*,
                                 SARC_STORAGE, struct ARCHandle*); // at 0xC
    virtual UNKTYPE* openDvdArchive(const char*,
                                    nw4r::snd::SoundHeap*); // at 0x10
    virtual UNKTYPE* openNandArchive(const char*,
                                     nw4r::snd::SoundHeap*); // at 0x14
    // openCntArchive: Unofficial arguments
    virtual UNKTYPE* openCntArchive(const char*, struct ARCHandle*,
                                    nw4r::snd::SoundHeap*); // at 0x18
    virtual UNKTYPE* setupMemoryArchive(const void*,
                                        nw4r::snd::SoundHeap*); // at 0x1C

    virtual UNKTYPE* setupMemoryArchive(const void* p,
    ...
    virtual bool prepareSound(nw4r::snd::SoundHandle* handle,
                              unsigned int id) // at 0x48
    {
        return ArcPlayer::prepareSound(handle, (u32)id);
    }

    virtual bool prepareSound(nw4r::snd::SoundHandle* handle,
                              const char* name) // at 0x4C
    {
        u32 id = -1;
        if (mOpenSndArchive)
            id = changeNameToId(name);
    ...
    virtual bool holdSound(nw4r::snd::SoundHandle* handle,
                           unsigned int id) // at 0x54
    {
        return ArcPlayer::holdSound(handle, (u32)id);
    }

    virtual bool holdSound(nw4r::snd::SoundHandle* handle,
                           const char* name) // at 0x58
    {
        u32 id = -1;
        if (mOpenSndArchive)
            id = changeNameToId(name);
