...
struct FrameCtrl {
    ...
    inline f32 GetFrm() const { return mFrame; }

    inline f32 GetRate() const { return mRate; }
    ...
    inline void SetRate(f32 rate) { mRate = rate; }

    inline void SetPolicy(PlayPolicyFunc func) { mPolicy = func; }

    inline void UpdateFrm() { SetFrm(mFrame + mRate * smBaseUpdateRate); }
...
struct AnmObj : G3dObj {
    enum AnmFlag { ANMFLAG_ISBOUND = 0x4 };
