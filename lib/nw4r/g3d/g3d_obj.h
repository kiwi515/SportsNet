...
#define NW4R_G3D_TYPE_OBJ_DECL(VAL)                                            \
    static const G3dObj::ResNameDataT<sizeof(#VAL)> TYPE_NAME
#define NW4R_G3D_TYPE_OBJ_DEF(VAL)                                             \
    const G3dObj::ResNameDataT<sizeof(#VAL)> VAL::TYPE_NAME = {sizeof(#VAL),   \
                                                               #VAL}
...
struct G3dObj {
    ...
    struct TypeObj {
        ...
        // It also makes sense because templates need to be resolved at
        // compile-time, so this is a way to store a pointer to one
        struct TypeObjData {
            u32 mLength;  // at 0x0
            char mName[]; // at 0x4
        };
        ...
        template <u32 N>
        inline TypeObj(const ResNameDataT<N>& pRes)
            : mData((const TypeObjData*)&pRes) {}

        inline u32 GetTypeID() const { return (u32)mData; }

        inline const char* GetTypeName() const { return mData->mName; }
    ...
    inline G3dObj* GetParent() const { return mParent; }
