...
inline bool operator==(LinkListImpl::Iterator iter1,
                       LinkListImpl::Iterator iter2) {
...
public:
    struct Iterator {
        ...
        T* operator->() const { return GetPointerFromNode(mIterator.mPointer); }
    ...
    Iterator Insert(Iterator iter, T* p) {
        return Iterator(
            LinkListImpl::Insert(iter.mIterator, GetNodeFromPointer(p)));
    }

    UNKTYPE PushBack(T* p) { Insert(GetEndIter(), p); }
