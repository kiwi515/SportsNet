...
public:
    ...
    ut::Font* GetFont() const { return mFont; }
...
public:
    ...
    void RegistFont(FontRefLink* pLink) { mRefList.PushBack(pLink); }
    void UnregistFont(FontRefLink* pLink) { mRefList.Erase(pLink); }
...
UNKTYPE* GetResourceSub(ARCHandle* pHandle, const char* rootDir, u32 filetype,
                        const char* filename, u32* pSize) {
    s32 entrynum = -1;

    if (ARCConvertPathToEntrynum(pHandle, rootDir) != -1 &&
        ARCChangeDir(pHandle, rootDir)) {
